...
CLIENT_OUT := tests/Katasec.OaiServer.Tests/Client
CLIENT_NS  := Katasec.OaiServer.Tests.Client

.PHONY: update-spec generate-client test
...
generate-client:
	@echo "Generating Kiota client..."
	@kiota generate \
		--language CSharp \
		--openapi $(SPEC_FILE) \
		--output $(CLIENT_OUT) \
		--class-name OpenAiClient \
		--namespace-name $(CLIENT_NS) \
		--include-path "/v1/chat/completions" \
		--include-path "/v1/responses" \
		--include-path "/v1/models" \
		--clean-output
	@echo "Client generated in $(CLIENT_OUT)"
